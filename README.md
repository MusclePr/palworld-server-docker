...
Wine-based mod sync uses `WORKSHOP_MOD_IDS` or `/palworld/workshop-mods.txt`, with `WORKSHOP_MOD_UPDATE_CRON` for periodic checks.
UE4SS support can be enabled through `INSTALL_UE4SS_EXPERIMENTAL=true`, which downloads and extracts the configured package into `/palworld/Mods/NativeMods/ue4ss-experimental` and deploys its runtime files at sync time.
In `SERVER_PLATFORM=Windows`, the default Wine prefix is `/palworld/.wine` so runtime dependencies installed by winetricks can persist with the `/palworld` volume.
For paid/private Workshop items, use a persisted Steam session (`/home/steam/Steam`) with `STEAM_USERNAME`. The helper script at [examples/mods/steam-login.sh](examples/mods/steam-login.sh) performs one-time interactive login and stores the login user marker in the session volume.
...
| WORKSHOP_MOD_IDS                           | Comma-separated Steam Workshop mod IDs to download and deploy.                                                                                          |                                                                                                    | Comma-separated numeric IDs                                                                                        | 2.8.0            |
| WORKSHOP_MOD_UPDATE_CRON                   | Cron expression for periodic Workshop mod sync checks. Leave empty to disable periodic checks.                                                          |                                                                                                    | Cron expression                                                                                                    | 2.8.0            |
| WORKSHOP_MODS_DEBUG                        | Enables verbose log output for mod sync actions.                                                                                                        | false                                                                                              | true/false                                                                                                         | 2.8.0            |
| STEAM_USERNAME                                | Steam account name used for authenticated Workshop downloads via persisted SteamCMD session cache.                                                       | anonymous                                                                                          | string                                                                                                             | 2.8.0            |
| STEAM_SESSION_VOLUME                       | Host path mounted to `/home/steam/Steam` to persist Steam Guard/SteamCMD login session for Workshop sync.                                              | ./Steam                                                                                            | path                                                                                                               | 2.8.0            |
| INSTALL_UE4SS_EXPERIMENTAL                 | Enables experimental UE4SS package download and deployment via NativeMods.                                                                              | false                                                                                              | true/false                                                                                                         | 2.8.0            |
| UE4SS_EXPERIMENTAL_URL                     | Download URL for the experimental UE4SS package zip.                                                                                                    | https://github.com/Okaetsu/RE-UE4SS/releases/download/experimental-palworld/UE4SS-Palworld.zip | URL                                                                                                                | 2.8.0            |
| UE4SS_MODS_LAYOUT                          | Runtime Mods deployment layout. `legacy` deploys to `Pal/Binaries/Win64/Mods`, `ue4ss_dir` deploys to `Pal/Binaries/Win64/ue4ss/Mods`.               | legacy                                                                                             | legacy, ue4ss_dir                                                                                                  | 2.8.0            |
| UE4SS_CLEANUP_LEGACY                       | Removes previously deployed legacy UE4SS runtime files before each sync to avoid stale-loader conflicts.                                                | true                                                                                               | true/false                                                                                                         | 2.8.0            |
