...
APComm_API() {
    ...
    curl -sS --fail --max-time 10 --retry 2 --retry-delay 1 -L -X POST "${url}" -H "${accept}" -A "${agent}" --json "${data}"
}
...
APComm_register() {
    ...
    if ! response=$(APComm_API "server/register" "${data}"); then
        APLog_error "server/register API call failed"
        return 1
    fi
...
APComm_update() {
    ...
    if ! response=$(APComm_API "server/update" "${APComm_jsonUpdate}"); then
        APLog_error "server/update API call failed"
        return 1
    fi
