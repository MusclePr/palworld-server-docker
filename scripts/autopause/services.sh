...
AutoPause_preSave() {
    ...
    if ! isTrue "${REST_API_ENABLED}"; then
        if result=$(REST_API save) && [ "${result}" = "" ]; then
            echo -n "OK"
            return 0;
        fi
        ...
        result="Set REST_API_ENABLED or RCON_ENABLED to true."
    fi
    echo -n "${result}"
    return 1;
}
...
AutoPause_challengeToPause() {
    local result
    result=$(AutoPause_preSave)
    APLog "Challenge to save before pause... ${result}"
    if [ "${result}" = "OK" ]; then
        if AP_pause on; then
            return 0
        fi
