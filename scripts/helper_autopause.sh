...
AutoPauseEx_resume() {
    ...
    if AP_isPaused; then
        if [ -n "${1}" ]; then APLog "Resumed by ${1}."; fi
        AP_pause off
    fi
}
...
AutoPauseEx_stopService() {
    ...
    if isTrue "${on}"; then
        ...
        if AP_isSkipped; then
            APLog_debug "Service has already disabled${by}."
        else
            APLog "Service has been disabled${by}."
        fi
    else
        if AP_isSkipped; then
            APLog "Service has been enabled${by}."
        else
            APLog_debug "Service has already enabled${by}."
        fi
    fi
