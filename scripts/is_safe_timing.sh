#!/bin/bash
# Save data is written every ${AUTO_SAVE_SPAN:-30} seconds and takes a few seconds to complete.
...
#   2 ... no timing by configuration (AUTO_SAVE_SPAN <= SG_NG_TIME is no timing.)

declare -i -r SG_LONGEST_TIME_TO_SAVE=${SAVEGAME_LONGEST_TIME:-5}
declare -i -r SG_MERGIN=${SAVEGAME_MERGIN_TIME:-5}
declare -i -r SG_NG_TIME=$((SG_LONGEST_TIME_TO_SAVE+SG_MERGIN))
declare -i -r SG_SPAN=${AUTO_SAVE_SPAN:-30}
declare -i -r SG_MIN=${SG_LONGEST_TIME_TO_SAVE}
declare -i -r SG_MAX=$((SG_SPAN-SG_MERGIN))

if [ ${SG_SPAN} -le ${SG_NG_TIME} ]; then
    echo "No timing, AUTO_SAVE_SPAN:${SG_SPAN} must be grater than ${SG_NG_TIME}."
    exit 2;
fi
if [ ${SG_MAX} -lt 0 ]; then SG_MAX=${SG_SPAN}; fi
...
SaveGames_isSafeTimimg() {
    ...
    if [ -n "${path}" ]; then
        now=$(date '+%s')
        ((d=now-mtime))
        if [ ${d} -ge ${SG_MIN} ] && [ ${d} -lt ${SG_MAX} ]; then
            echo "OK"
            return 0
        fi
        local -i limit=$((SG_SPAN * 2))
        if [ ${d} -ge ${limit} ]; then
